...
<div class="crm-block crm-form-block crm-paymentProcessor-form-block">
    ...
    <table class="form-layout-compressed">
        <tr class="crm-paymentProcessor-form-block-payment_processor_type">
            <td class="label">{$form.payment_processor_type_id.label}</td>
            <td>{$form.payment_processor_type_id.html} {help id='proc-type'}</td>
        </tr>
        ...
        <tr class="crm-paymentProcessor-form-block-financial_account">
            ...
            <td>
                ...
                {else}
                    ...
                    {ts 1=$ftUrl}There are no financial accounts configured with Financial Account Type 'Asset' Type.
                        <a href='%1'>Click here</a>
                        if you want to configure financial accounts for your site.{/ts}
                {/if}
            </td>
        </tr>
        ...
        <tr class="crm-paymentProcessor-form-block-is_default">
            <td></td>
            <td>{$form.is_default.html}&nbsp;{$form.is_default.label}</td>
        </tr>
        {if $form.recur_cycleday}
            <tr class="crm-paymentProcessor-form-block-recur_cycleday">
                <td class="label">{$form.recur_cycleday.label}</td>
                <td>{$form.recur_cycleday.html}
                    <p class="description">{ts}The default date (day of month) on which monthly recurring transactions are processed.{/ts}
                        <br/>
    ...
    <fieldset>
        ...
        <table class="form-layout-compressed">
            <tr class="crm-paymentProcessor-form-block-user_name">
                <td class="label">{$form.user_name.label}</td>
                <td>{$form.user_name.html} {help id=$ppTypeName|cat:'-live-user-name' title=$form.user_name.label}</td>
            </tr>
            {if $form.password}
                <tr class="crm-paymentProcessor-form-block-password">
                    <td class="label">{$form.password.label}</td>
                    <td>{$form.password.html} {help id=$ppTypeName|cat:'-live-password' title=$form.password.label}</td>
                </tr>
            {/if}
            {if $form.signature}
                <tr class="crm-paymentProcessor-form-block-signature">
                    <td class="label">{$form.signature.label}</td>
                    <td>{$form.signature.html} {help id=$ppTypeName|cat:'-live-signature' title=$form.signature.label}</td>
                </tr>
            {/if}
        </table>
    </fieldset>
    ...
    <fieldset>
        ...
        <table class="form-layout-compressed">
            <tr class="crm-paymentProcessor-form-block-test_user_name">
                <td class="label">{$form.test_user_name.label}</td>
                <td>{$form.test_user_name.html} {help id=$ppTypeName|cat:'-test-user-name' title=$form.test_user_name.label}</td>
            </tr>
            {if $form.test_password}
                <tr class="crm-paymentProcessor-form-block-test_password">
                    <td class="label">{$form.test_password.label}</td>
                    <td>{$form.test_password.html} {help id=$ppTypeName|cat:'-test-password' title=$form.test_password.label}</td>
                </tr>
            {/if}
            {if $form.test_signature}
                <tr class="crm-paymentProcessor-form-block-test_signature">
                    <td class="label">{$form.test_signature.label}</td>
                    <td>{$form.test_signature.html} {help id=$ppTypeName|cat:'-test-signature' title=$form.test_signature.label}</td>
                </tr>
            {/if}
            {/if}
        </table>
