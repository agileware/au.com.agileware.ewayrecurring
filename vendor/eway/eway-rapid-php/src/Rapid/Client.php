...
class Client implements ClientContract {
  ...
  /**
   * Possible values ("Production", "Sandbox", or a URL) "Production" and
   * "Sandbox" will default to the Global Rapid API Endpoints.
   *
   * @var string
   */
  private $endpoint;
    ...
    switch ($apiMethod) {
      ...
      case ApiMethod::WALLET:
        ...
        return $this->getHttpService()
          ->postTransaction($transaction->toArray());
      ...
      case ApiMethod::RESPONSIVE_SHARED:
        ...
        return $this->getHttpService()
          ->postAccessCodeShared($transaction->toArray());
      ...
      case ApiMethod::AUTHORISATION:
        return $this->getHttpService()
          ->postCapturePayment($transaction->toArray());
  ...
  private function doQueryInvoiceReference($invoiceReference) {
    return $this->getHttpService()
      ->getTransactionInvoiceReference($invoiceReference);
  }
  ...
  private function doRefund($refund) {
    ...
    return $this->getHttpService()
      ->postTransactionRefund($refund->Refund->TransactionID, $refund->toArray());
  }
