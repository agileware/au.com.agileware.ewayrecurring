...
interface Client {
  ...
  /**
   * Call to change the base API URL the Rapid Client is using to communicate
   * with Rapid API Can be MODE_PRODUCTION, MODE_SANDBOX or a URL
   *
  ...
  /**
   * Called to change the credentials the Rapid Client is using to communicate
   * with Rapid API
   *
   * @param string $apiKey
  ...
  /**
   * This Method is used to create a transaction for the merchant in their eWAY
   * account.
   *
   * Depending on the PaymentMethod parameter specified, the transaction may be
   * created immediately, or it may be pending (waiting for Card Details to be
   * supplied via the Responsive Shared Page, or Transparent Redirect).
   *
   * If the 'Capture' flag on the Transaction is true then the funds for the
   * Transaction will be debited immediately from the cardholder (default
   * behaviour). If the 'Capture' flag is false then an Authorisation is
   * created instead. Authorisation is not available for NZ and UK merchants.
   *
   ...
   * It's primarily of use for PaymentMethods TransparentRedirect and
   * ResponsiveShared, to interrogate Rapid once the transaction is complete.
   *
  ...
  /**
   * This Method is used to create a token customer for the merchant in their
   * eWAY account. The token customer can be used to create MOTO or Recurring
   * transactions at a later time.
   *
   * Like the CreateTransaction, a PaymentMethod is specified which determines
   * what method will be used to capture the card that will be saved with the
   * customer. Depending on the PaymentMethod the customer may be created
   * immediately, or it may be pending (waiting for Card Details to be supplied
   * by the Responsive Shared Page, or Transparent Redirect).
   *
   * @param string $apiMethod
  ...
  /**
   * This Method is used to update a existing token customer for the merchant
   * in their eWAY account. Card, email and address changes can be made.
   *
   * Like the CreateCustomer, a PaymentMethod is specified which determines
   * what method will be used to capture the card that will be saved with the
   * customer. Depending on the PaymentMethod the customer may be updated
   * immediately, or it may be pending (waiting for Card Details to be supplied
   * by the Responsive Shared Page, or Transparent Redirect).
   *
   * The SDK will use the PaymentMethod parameter to determine what type of
   * transaction to create as per the transaction type mapping table in the
   * Create (Transaction) API Method spec (above).
   *
   * @param string $apiMethod
   * @param Customer|array $customer
   *
  ...
  /**
   * This method is used to return the details of a Token Customer. This
   * includes masked Card information for displaying in a UI to a user.
   *
   ...
   *
   * @param string|int $transactionId Transaction Id of the transaction to be
   *   cancel
   *
   * @return RefundResponse
   */
